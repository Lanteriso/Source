...
ULyraAbilitySystemComponent* URPGAbilityTask_PlayMontageAndWaitForEvent::GetTargetASC()
{
	return Cast<ULyraAbilitySystemComponent>(AbilitySystemComponent);
}
...
void URPGAbilityTask_PlayMontageAndWaitForEvent::Activate()
{
	...
	ULyraAbilitySystemComponent* LyraAbilitySystemComponent = GetTargetASC();
	...
	if (LyraAbilitySystemComponent)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = LyraAbilitySystemComponent->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &URPGAbilityTask_PlayMontageAndWaitForEvent::OnGameplayEvent));

			if (LyraAbilitySystemComponent->PlayMontage(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection) > 0.f)
			{
...
void URPGAbilityTask_PlayMontageAndWaitForEvent::OnDestroy(bool AbilityEnded)
{
	...
	if (LyraAbilitySystemComponent)
	{
		LyraAbilitySystemComponent->RemoveGameplayEventTagContainerDelegate(EventTags, EventHandle);
	}
